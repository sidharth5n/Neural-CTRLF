...
class PreActResNet(nn.Module):
    ...
    def _make_layer(self, block, features, count, stride, first = False):
        ...
        for i in range(count):
            s.append(block(self.in_channels, features, stride if i==0 else 1, first and i == 0))
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_channels, n, stride, first):
        ...
        if not first:
            self.first = nn.Sequential(nn.BatchNorm2d(in_channels), 
                                       nn.ReLU())

        self.layers = nn.Sequential(nn.Conv2d(in_channels, n, 3, stride, 1),
                                    nn.BatchNorm2d(n),
                                    nn.ReLU(),
                                    nn.Conv2d(n, n, 3, 1, 1))

        if stride != 1 or in_channels != n*self.expansion:
            self.shortcut = nn.Sequential(nn.Conv2d(in_channels, n*self.expansion, 1, stride),
                                          nn.BatchNorm2d(n*self.expansion))

    def forward(self, x):
        x = self.first(x) if hasattr(self, 'first') else x
        x1 = self.layers(x)
...
class BottleNeck(nn.Module):
    ...
    def __init__(self, in_channels, n, stride, first):
        ...
        self.layers = nn.Sequential(nn.Conv2d(in_channels, n, 1, 1),
                                    nn.BatchNorm2d(n),
                                    nn.ReLU(),
...
if __name__ == '__main__':
    x = torch.randn((1, 1, 1720, 800))
    m = PreActResNet18()#.model[:6]
    # print(m)
    print(m(x).shape)
    # print()
